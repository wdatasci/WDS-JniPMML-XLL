...
/* <<< C# */
    ...
    public class WranglerFlatFile
    {
        ...
        //Java
        public CSVParser __CSVParser = null;
        //Java
        public Iterator<CSVRecord> __CSVParserIterator = null;
        //C# public StreamReader __StreamReader = null;
        //C# public CsvParser __CSV = null;
        ...
        throws com.WDataSci.WDS.WDSException
        {
            ...
            aInputRecordSetMD.FileMatter.FileName = aFileName;
        ...
        throws com.WDataSci.WDS.WDSException
        {
            ...
            try {
                ...
                //C# Boolean bCheckingAgainstPMML = false;

                //Java
                org.dmg.pmml.DataField[] lDataFields = null;
                ...
                //Java
                int xnlLength = inputLine.size();
                //C# int xnlLength = inputLine.Length;
                ...
                for ( ii = 0 ; ii < xnlLength ; ii++ ) {
                    ...
                    //Java
                    cm.Name = inputLine.get(ii);
                    //C# cm.Name = inputLine[ii];
        ...
        throws com.WDataSci.WDS.WDSException
        {
            try {
                ...
                if ( aInputRecordSetMD.FileMatter == null )
                    aInputRecordSetMD.FileMatter.cPointToFile(aInputRecordSetMD, aFileName, hasHeaderRow, dlm);
                ...
                //Java
                if ( this.__CSVParser == null || this.__CSVParser.isClosed() || !this.Path.equals(aPath) || !this.FileName.equals(aFileName) )
                //C# if ( this.__CSV == null || !this.Path.equals(aPath) || !this.FileName.equals(aFileName) )
                {
                ...
                if ( aInputRecordSet.isEmpty() ) {
                    ...
                    //C# aInputRecordSet.Records = new List<Map<FieldName, Object>>(0);
                    //Java
                ...
                //Java
                if ( hasHeaderRow && this.__CSVParser.getCurrentLineNumber() == 0 )
                //C# if ( hasHeaderRow && this.__CSV.Context.Row < 1 )
                {
                    //Java
                    inputLine = this.__CSVParserIterator.next();
                    //C# inputLine = this.__CSV.Read();
                }
                int row = -1;

                //Java
                while ( this.__CSVParserIterator.hasNext() )
                //C# while (true)
                {
                    //Java
                    inputLine = this.__CSVParserIterator.next();
                    ...
                    if ( nIncomingFields > nInputMap )
                        //Java
                        throw new com.WDataSci.WDS.WDSException(String.format("Error reading from delimited file, row %d has insufficient columns, stopping input!\n", row + 1));
                        //C# throw new com.WDataSci.WDS.WDSException("Error reading from delimited file, row " + (row + 1) + " has insufficient columns, stopping input!\n");
                    ...
                    for ( j = 0 ; j < nInputMap ; j++ ) {
                        ...
                        if ( s == null || s.isEmpty() ) {
                            ...
                            //Java
                            Double lv = Double.parseDouble(inputLine.get(j));
                            /* C# >>> *
                            ...
                            //Java
                            Long lv = Long.parseLong(inputLine.get(j));
                            /* C# >>> *
                            ...
                            //Java
                            Integer lv = Integer.parseInt(inputLine.get(j));
                                /* C# >>> *
                            ...
                            //C# String lv = com.WDataSci.WDS.Util.CleanAsToken(inputLine.get(j));
                            inputRow_orig[j] = lv;
                            if ( aInputRecordSetMD.Column[j].hasMapKey() )
                                inputRow.put(aInputRecordSetMD.Column[j].MapKey, lv);
                        }
                ...
                //Java
                this.__CSVParser.close();
                //C# this.__CSV.Dispose();
        ...
        throws com.WDataSci.WDS.WDSException
        {
            ...
            if ( aOutputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.CSV, RecordSetMDEnums.eType.TXT, RecordSetMDEnums.eType.Dlm) ) {
                try {
                    ...
                    //Java
                    BufferedWriter outBW = Files.newBufferedWriter(Paths.get(aPathAndName));
                    //C# StreamWriter outBW = new StreamWriter(aPathAndName);

                    //Java
                    CSVPrinter outCSV = null;
                    //C# CsvWriter outCSV=null;
                    ...
                    for ( i = 0; i < aInputRecordSet.Records.size(); i++ ) {
                        //Java
                        List<String> outputrow = new ArrayList<>(0);
                        //C# List<String> outputrow = new List<String>(0);
                        jj = 0;
                        if ( aOutputRecordSetMD.ModeMatter.bRepeatInputFields ) {
                            //Java
                            for ( Object obj : aInputRecordSet.Records_Orig.get(i) )
        ...
        throws com.WDataSci.WDS.WDSException
        {
            ...
            if ( aOutputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.CSV, RecordSetMDEnums.eType.TXT, RecordSetMDEnums.eType.Dlm) ) {
                try {
                    ...
                    //Java 
                    BufferedWriter outBW = Files.newBufferedWriter(Paths.get(aOutputRecordSetMD.FileMatter.FileName));
                    //C# StreamWriter outBW = new StreamWriter(aOutputRecordSetMD.FileMatter.FileName);
        ...
        public void mWriteRecordSet(RecordSetMD aOutputRecordSetMD, MOIE.ListObject aListObject)
        {
            ...
            if ( aOutputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.CSV, RecordSetMDEnums.eType.TXT, RecordSetMDEnums.eType.Dlm) ) {
                try {
                    ...
                    StreamWriter outBW = new StreamWriter(aOutputRecordSetMD.FileMatter.FileName);
